...
export function splitBankStatementContent(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isTransactionLine(line)) {
      ...
      if (
        transactionCount > maxTransactionsPerBatch &&
  ...
  console.log(
    `Split content into ${chunks.length} chunks with ~${maxTransactionsPerBatch} transactions each`,
  );
...
export function estimateTransactionCount(content: string): number {
  ...
  for (const line of lines) {
    ...
    const matches = transactionPatterns.filter((pattern) =>
      pattern.test(line),
    ).length;
    if (matches >= 2) {
      count++;
...
export function mergeBatchResults(results: BatchResult[]): BatchResult {
  ...
  const totalIncome = results.reduce(
    (sum, r) => sum + r.summary.totalIncome,
    0,
  );
  const totalExpenses = results.reduce(
    (sum, r) => sum + r.summary.totalExpenses,
    0,
  );

  // Find overall date range
  const allDates = results.flatMap((r) => [
    r.summary.dateRange.start,
    r.summary.dateRange.end,
  ]);
